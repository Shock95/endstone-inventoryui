...
class InventoryContentPacket(Packet):
    container_id: int = 0
    items: list[ItemStackWrapper] = field(default_factory=list)
    container_name: FullContainerName = field(default_factory=FullContainerName)
    storage: ItemStackWrapper = field(default_factory=ItemStackWrapper)
